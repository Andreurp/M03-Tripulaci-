...
public class SampleController implements Initializable{
	...
	private void genera(){
		...
		for(int i=0; i<100;i++){
			...
			for(int t=0; t<tripulants; t++){
				
				Tripulant c = new Tripulant();
				c.setNom(noms[r.nextInt(noms.length)]);
				do{
					c.setRang(rang[r.nextInt(rang.length)]);
				}while(hihaCapita==true && c.getRang().equals("capita"));
				if(c.getRang().equals("capita")){
					hihaCapita=true;
				}
				c.setVaxeill(v);
